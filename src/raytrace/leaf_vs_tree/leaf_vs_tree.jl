function traverse_rays(
    ...
    if AK.get_backend(bvh.nodes) isa typeof(AK.CPU_BACKEND)
        ...
        # GPU backend - one per input ray
        npadding = 1
        thread_ncontacts_length = size(points, 2)
    end
    ...
    traverse_rays_lvt!(bvh, points, directions, start_level, thread_ncontacts, nothing, narrow, options)
    ...
    traverse_rays_lvt!(bvh, points, directions, start_level, thread_ncontacts, contacts, narrow, options)

    # Return contact list and the other buffer as possible cache
    BVHTraversal(
        Int(start_level), 0, Int(total_contacts),
        contacts, thread_ncontacts_raw,
    )
end


function traverse_rays_lvt!(
    bvh::BVH,
    points::AbstractArray, directions::AbstractArray,
    start_level::Int,
    ...
    if backend isa typeof(AK.CPU_BACKEND)
        AK.itask_partition(num_checks, options.num_threads, options.min_traversals_per_thread) do itask, irange
            ...
            for i in irange
                ...
                direction = T.((
                    @inbounds(directions[1, i]),
                    @inbounds(directions[2, i]),
                    @inbounds(directions[3, i])
                ))
                @inline traverse_ray_lvt!(
                    point, direction, I(i),
                    bvh, stack,
...
function traverse_ray_lvt!(
    point::NTuple{3, T}, direction::NTuple{3, T}, iray::I,
    ...
    # Traverse a single ray against the BVH using ray-vs-tree traversal; descend
    ...
    @inbounds for inode_root in inode_start:inode_end
        ...
        while true
            ...
            if ilevel == bvh.tree.levels
                ...
                if isintersection(leaf.volume, point, direction) && narrow(leaf, point, direction)
                    if isnothing(contacts)
                        # Counting pass - known at compile-time
                        @inbounds thread_ncontacts[iwrite[]] += I(1)
                    else
                        # Writing pass
                        @inbounds contacts[iwrite[]] = (leaf.index, iray)
                        iwrite[] += I(1)
                    end
                end
            else
                node = @inbounds bvh.nodes[inode - bvh.skips[ilevel]]
                if isintersection(node, point, direction)
